...
type ColumnType = ColumnDef<ResidentInterface, unknown>;
...
export function ResidentListPage(props: ResidentListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getResidents({
        ...
        order: params.order,
        searchTermKeys: ['first_name.contains', 'last_name.contains', 'gender.contains', 'medical_condition.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<ResidentInterface>>(
    () => `/residents?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const columns: ColumnType[] = [
    ...
    { id: 'medical_condition', header: 'Medical Condition', accessorKey: 'medical_condition' },
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/residents/view/${record.id}`} passHref legacyBehavior>
                <Button
                  onClick={(e) => e.stopPropagation()}
                  mr={2}
                  padding="0rem 8px"

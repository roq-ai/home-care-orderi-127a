...
function DashboardPage() {
  ...
  const residentStat = useSWR<PaginatedInterface<ResidentInterface>>(
    () => `/residents`,
    () => getResidents({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Residents',
      result: residentStat?.data?.totalCount,
      isLoading: residentStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Items',

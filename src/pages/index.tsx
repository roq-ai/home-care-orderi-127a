...
function HomePage() {
  ...
  return (
    <>
      <Head>
        <title>{`Home care ordering system`}</title>

        <meta
          name="description"
          content="Experience seamless healthcare with Home Care Ordering System - Your comprehensive tool for efficient medication ordering and management for home-care institutions."
        />
      </Head>
      <ConfigureCodeBanner
      ...
      <HomeLayout>
        <Box position="relative" display="flex" flex={{ lg: '0 0 485px' }} height={{ base: '180px', lg: 'auto' }}>
          <Image
            flex="1"
            src={
              'https://images.unsplash.com/photo-1585435557343-3b092031a831?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w0NjA3NjB8MHwxfHNlYXJjaHwxfHxwaGFybWFjeXxlbnwwfHx8fDE2OTM1NTkzNDN8MA&ixlib=rb-4.0.3&q=80&w=1080'
            }
            alt="Dinner"
            objectFit="cover"
          ...
          <Box position="absolute" top="0" backgroundColor="rgba(0,0,0,0.6)" width="100%" py="2">
            <Text align="center" fontSize="sm" color="white">
              Photo by{' '}
              <Link
                href="https://unsplash.com/@lunarts?utm_source=roq-generator&utm_medium=referral"
                isExternal
                color="teal.200"
              >{`Volodymyr Hryshchenko`}</Link>{' '}
              on{' '}
              <Link
        ...
        >
          <Box>
            <Heading mb={{ base: '12px', lg: '20px' }} size="2xl">
              {`Home care ordering system`}
            </Heading>
            <Text size="2xl" mb={{ base: '32px', lg: '48px' }} color="base.content" opacity="0.6">
              {`Experience seamless healthcare with Home Care Ordering System - Your comprehensive tool for efficient medication ordering and management for home-care institutions.`}
            </Text>
            <Box
            ...
            >
              <Stack direction={{ md: 'row', base: 'column' }} spacing="1.5rem">
                <Card
                  userTypeName="Home Care Administrator"
                  type="home-care-administrator"
